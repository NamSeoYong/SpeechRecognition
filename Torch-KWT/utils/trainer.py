...
@torch.no_grad()
def evaluate(net: nn.Module, criterion: Callable, dataloader: DataLoader, device: torch.device) -> Tuple[float, float, float]:
    """Performs inference and calculates accuracy, loss, and ROC-AUC.
...
def evaluate_roc_auc(model, dataloader, device):
    model.eval()
    y_true = []
    y_scores = []
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            scores = torch.softmax(outputs, dim=1)[:, 1]  # 예측 클래스가 1일 확률을 사용합니다.
...
def train(net: nn.Module, optimizer: optim.Optimizer, criterion: Callable, trainloader: DataLoader, valloader: DataLoader, schedulers: dict, config: dict) -> None:
    ...
    for epoch in range(config["hparams"]["n_epochs"]):
        ...
        if not epoch % config["exp"]["val_freq"]:
            val_acc, avg_val_loss, roc_auc = evaluate(net, criterion, valloader, device)
            log_dict = {"epoch": epoch, "val_loss": avg_val_loss, "val_acc": val_acc, 'roc_auc': roc_auc}
            log(log_dict, step, config)
