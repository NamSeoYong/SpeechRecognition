...
def get_clip_pred(net, audio_path, win_len, stride, thresh, config, batch_size, device, mode, label_map) -> list:
    ...
    if len(result):
      ...
      if mode == "max":
            max_conf_idx = result[:, 1].argmax()
            pred_class = label_map.get(str(int(result[max_conf_idx, 0])))
            if pred_class is not None:
                pred = [pred_class, result[max_conf_idx, 1], result[max_conf_idx, 2]]
            else:
                pred = [result[max_conf_idx, 0], result[max_conf_idx, 1], result[max_conf_idx, 2]]
      elif mode == "n_voting":
            preds = result[:, 0].astype(int)
            confs = result[:, 1]
            starts = result[:, 2]
            votes = np.bincount(preds)
            max_vote = np.argmax(votes)
            max_vote_conf = confs[preds == max_vote].max()
            max_vote_start = starts[preds == max_vote].min()
            pred = [max_vote, max_vote_conf, max_vote_start]
            if label_map is not None:
                pred[0] = label_map[str(int(pred[0]))]
      elif mode == "multi":
          if label_map is not None:
